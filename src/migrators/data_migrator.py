...
class DataMigrator:
    ...
    def _get_mount_info(self, path: Path) -> dict:
        ...
        try:
            ...
            result = subprocess.run(["findmnt", "-n", "-o", "FSTYPE,OPTIONS", str(path)], capture_output=True, text=True, check=True)

            parts = result.stdout.strip().split()
            if len(parts) >= 2:
                return {"filesystem": parts[0], "options": parts[1].split(",")}
                ...
                with open("/proc/mounts", "r") as f:
                    for line in f:
                        ...
                        if len(parts) >= 4 and parts[1] == str(path):
                            return {"filesystem": parts[2], "options": parts[3].split(",")}
    ...
    def move_data(self, dry_run: bool = False) -> dict:
        ...
        if not needs_migration:
            return {"success": False, "method": "none", "reason": reason, "dry_run": dry_run}
    ...
    def _move_with_mv(self, source: Path, destination: Path) -> dict:
        ...
        try:
            ...
            return {"operation": "move", "command": f"mv {source} {destination}", "atomic": True}
    ...
    def _move_with_rsync(self, source: Path, destination: Path) -> dict:
        ...
        try:
            ...
            return {"operation": "rsync", "command": " ".join(rsync_cmd), "atomic": False, "output": result.stdout, "stderr": result.stderr}
